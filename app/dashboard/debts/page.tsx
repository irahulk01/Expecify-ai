      ...
      <main className="px-6 md:px-10 py-8 space-y-8 animate-fade-in-up">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-text-secondary font-medium tracking-tight opacity-70">
              Track and manage your loans, EMIs, and credit balances.
            </p>
          </div>
        </div>
          ...
          <div className="bg-surface border border-border rounded-3xl p-6 shadow-xl animate-in slide-in-from-top-4 fade-in duration-300">
            ...
            <form onSubmit={handleAddDebt} className="flex gap-4 items-end">
              ...
              <button
                ...
                type="submit"
                className="px-6 py-3 bg-white text-black font-bold rounded-xl active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {mutation.isPending ? (
                  <>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="h-48 rounded-3xl bg-surface animate-pulse border border-border"
              ></div>
            ))}
          </div>
        ) : debts.length === 0 ? (
          <div className="text-center py-20 px-6 bg-surface border border-border border-dashed rounded-3xl">
            ...
            <p className="text-sm text-text-secondary max-w-sm mx-auto mb-6">
              You currently have no active loans or credit balances documented. Add one above to
              start tracking it.
            </p>
          </div>
        ) : (
          <div className="flex overflow-x-auto hide-scrollbar snap-x snap-mandatory -mx-6 px-6 pb-6 gap-6 md:grid md:grid-cols-2 md:mx-0 md:px-0 md:pb-0">
            {debts.map((debt) => {
              ...
              return (
                <div
                  key={debt.id}
                  className="relative bg-surface border border-border rounded-3xl p-6 hover:shadow-lg transition-all group shrink-0 w-[85%] md:w-auto snap-center"
                >
                    ...
                    <div className="relative pt-2">
                      ...
                      <div className="overflow-hidden h-2.5 mb-4 text-xs flex rounded-full bg-background border border-border inset-shadow-sm">
                        <div
                          ...
                          className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-linear-to-r from-emerald-500 to-emerald-400"
                        ></div>
                      </div>
                    </div>
                  </div>
                  ...
                  {deleteConfirmId === debt.id && (
                    <div className="absolute inset-0 bg-background/80 backdrop-blur-sm rounded-3xl z-40 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-200">
                      ...
                      <p className="text-sm text-text-secondary max-w-[200px] mb-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          disabled={deleteMutation.isPending}
                          className="px-4 py-2 text-sm font-bold text-white bg-rose-500 rounded-xl hover:bg-rose-600 transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2"
                        >
