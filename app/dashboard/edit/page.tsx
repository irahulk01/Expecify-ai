...
export default function EditDataPage() {
  ...
  const [activeTab, setActiveTab] = useState<"transactions" | "incomes" | "investments" | "debts">(
    "transactions"
  );
  ...
  // SVG Donut Logic
  const donutSegments = categories.reduce(
    (
      acc: (Category & {
        isFull: boolean;
        hexColor: string;
      ...
      const pathData = [`M ${x1} ${y1}`, `A 40 40 0 ${largeArcFlag} 1 ${x2} ${y2}`].join(" ");
  ...
  const PaginatedItem = ({ item }: { item: FinancialItem }) => (
    <div
      key={item.id}
      className="group flex justify-between items-center p-4 bg-[#0A0A0B] rounded-2xl border border-white/5 hover:border-white/10 transition-colors"
    >
      {editingId === item.id ? (
        <div className="flex items-center gap-3 w-full flex-wrap sm:flex-nowrap">
          ...
          <div className="flex gap-2 w-full sm:w-auto justify-end mt-2 sm:mt-0">
            <button
              ...
              onClick={() => handleSave(activeTab.slice(0, -1), item.id)}
              className="bg-emerald-500/20 text-emerald-400 p-2.5 hover:bg-emerald-500/30 rounded-xl transition-colors shadow-sm disabled:opacity-50"
            >
              {savingId === item.id ? (
            ...
            <button
              onClick={cancelEdit}
              className="bg-white/5 text-slate-400 p-2.5 hover:bg-white/10 rounded-xl transition-colors shadow-sm"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <>
          <div className="min-w-0 pr-4">
            ...
            {item.category && (
              <p className="text-[11px] font-bold tracking-widest uppercase text-slate-500 truncate">
                {item.category}
              </p>
            )}
          </div>
          <div className="flex items-center gap-4 shrink-0">
            <div className="text-right">
              <span
                className={`tabular-nums block font-black ${activeTab === "transactions" ? (item.type === "income" ? "text-emerald-400" : "text-slate-300") : "text-white"}`}
              >
            ...
            <div className="flex items-center gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="bg-white/5 text-primary hover:bg-primary/20 p-2 rounded-xl transition-colors tooltip-target"
              >
              ...
              <button
                ...
                className="bg-white/5 text-red-400 hover:bg-red-500/20 p-2 rounded-xl transition-colors tooltip-target disabled:opacity-50"
              >
                {deletingId === item.id ? (
  ...
  const TABS = [
    {
      id: "transactions",
      label: "Transactions",
      count: allItems.transactions.length,
      icon: Activity,
    },
      ...
      <header className="sticky top-0 z-40 bg-[#0A0A0B]/80 backdrop-blur-2xl border-b border-white/5 pt-6 pb-4 px-6 sm:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              href="/dashboard"
              className="w-10 h-10 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center hover:bg-white/10 transition-colors shadow-sm"
            >
              <ArrowLeft className="w-5 h-5 text-slate-300" />
            </Link>
            <div>
              <p className="text-[10px] text-primary font-bold tracking-widest uppercase mb-0.5">
                Control Center
              </p>
            ...
            <div className="w-full space-y-3">
              ...
              {categories.map((cat: any) => {
                ...
                return (
                  <div
                    key={cat.label}
                    className="flex justify-between items-center p-3 bg-[#0A0A0B] border border-white/5 rounded-2xl"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="w-3 h-3 rounded-full shadow-lg"
                        style={{ backgroundColor: hexColor, boxShadow: `0 0 10px ${hexColor}80` }}
                      />
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {tab.label}
                  <span
                    className={`px-2 py-0.5 rounded-md text-[10px] font-black ${activeTab === tab.id ? "bg-primary text-white" : "bg-white/10 text-slate-400"}`}
                  >
                    {tab.count}
                  </span>
                </button>
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 text-[10px] text-slate-600 font-medium tracking-tight">
