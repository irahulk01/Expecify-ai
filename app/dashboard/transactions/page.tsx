...
import { 
  Plus, Bell, Search, Calendar, ArrowUpRight, ArrowDownLeft, ChevronLeft, ChevronRight,
  TrendingDown, TrendingUp, Mic
} from "lucide-react";
import { 
      ...
      <main className="px-6 md:px-10 py-8 space-y-8 animate-fade-in-up">
            ...
            {["all", "income", "expense"].map((t) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                  filter === t ? "bg-primary text-white shadow-md" : "text-text-secondary hover:text-text-primary"
                }`}
              >
                {t}
              </button>
        ...
        <div className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm">
           <div className="flex items-center justify-between mb-8">
              <div>
                 <h3 className="font-black text-xl text-text-primary tracking-tight">Spending Visualization</h3>
                 <p className="text-xs font-bold text-text-secondary mt-1">Summary of outflows across the current period</p>
              </div>
                 ...
                 <AreaChart data={
                    [...Array(7)].map((_, i) => {
                       ...
                       const amount = transactions
                          .filter(tx => tx.type === 'expense' && new Date(tx.date).toLocaleDateString() === dateStr)
                    ...
                    <Tooltip 
                       contentStyle={{ backgroundColor: '#111827', border: 'none', borderRadius: '12px', color: '#F8FAFC', fontSize: '12px', fontWeight: 'bold' }}
                       formatter={(val: any) => `₹${Number(val).toLocaleString()}`}
                    />
                    <Area type="monotone" dataKey="amount" stroke="#F43F5E" strokeWidth={3} fillOpacity={1} fill="url(#colorAmountTx)" />
                 </AreaChart>
              </ResponsiveContainer>
           </div>
        </div>
        ...
        <div className="bg-surface border border-border rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col">
          ...
          <div className="divide-y divide-border/50 flex-1">
            ...
            {paginatedTx.map((tx) => (
              <div key={tx.id} className="grid grid-cols-12 px-8 py-6 items-center group hover:bg-background/40 transition-colors">
                <div className="col-span-6 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-text-secondary font-medium uppercase tracking-wide opacity-60">Success • {tx.type === 'income' ? 'Wallet Credit' : 'Merchant Debit'}</p>
                  </div>
                </div>
          ...
          {totalPages > 1 && (
            <div className="px-8 py-6 border-t border-border bg-background/30 flex items-center justify-between">
              <p className="text-xs font-bold text-text-secondary">
                Showing {Math.min(filteredTx.length, (currentPage - 1) * ITEMS_PER_PAGE + 1)} to {Math.min(filteredTx.length, currentPage * ITEMS_PER_PAGE)} of {filteredTx.length} results
              </p>
              <div className="flex items-center gap-2">
                <button 
                  ...
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl text-xs font-black transition-all ${
                        currentPage === i + 1 ? "bg-primary text-white shadow-md shadow-primary/20" : "text-text-secondary hover:bg-surface"
                      }`}
                    >
                      {i + 1}
                    </button>
