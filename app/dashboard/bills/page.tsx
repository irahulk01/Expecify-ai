...
import { 
  Plus, Bell, Search, Home, Zap, Tv, CreditCard, ChevronRight, AlertCircle, Clock, Phone
} from "lucide-react";
    ...
    <div className="min-w-0 transition-all">
      ...
      <main className="px-10 py-8 space-y-10">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black text-text-primary tracking-tight mb-2">Obligations & Bills</h1>
            <p className="text-sm text-text-secondary font-medium">Auto-tracked from your messages and linked accounts.</p>
          </div>
          <div className="flex flex-col items-end gap-3 font-mono">
            <div className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
               <span className="text-[10px] font-black uppercase text-emerald-500 tracking-widest">Paid Total: </span>
               <span className="text-sm font-black text-emerald-500 ml-2">₹{bills.filter(b => b.status === 'paid').reduce((sum, b) => sum + b.amount, 0).toLocaleString()}</span>
            </div>
              ...
              {["upcoming", "paid", "all"].map((t) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                    activeTab === t ? "bg-primary text-white shadow-md" : "text-text-secondary hover:text-text-primary"
                  }`}
                >
                  {t}
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBills.map((bill) => {
            ...
            return (
              <div key={bill.id} className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                <div className="flex items-start justify-between mb-8">
                  <div className={`w-14 h-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center`}>
                    <Icon className="w-7 h-7" />
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] uppercase font-black text-text-secondary tracking-widest mb-1">Due Date</p>
                    <p className="text-sm font-black text-text-primary">{new Date(bill.dueDate).toLocaleDateString("en-IN", { day: 'numeric', month: 'short' })}</p>
                  </div>
                </div>
                
                <div className="mb-8">
                  <h4 className="text-lg font-black text-text-primary mb-1 tracking-tight">{bill.title}</h4>
                  <p className="text-2xl font-black text-primary tracking-tighter">₹{bill.amount.toLocaleString()}</p>
                </div>
          ...
          <div className="border-2 border-dashed border-border rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center group cursor-pointer hover:border-primary/50 transition-all bg-linear-to-b from-surface to-background/50">
             ...
             <p className="text-[10px] font-bold text-text-secondary px-6">Connect your utility accounts for auto-imports.</p>
          </div>
        </div>
      </main>
    </div>
