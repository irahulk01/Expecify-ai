...
import { Plus, Bell, ShoppingBag, Mic, Zap, Target, Wallet, Plane } from "lucide-react";
      ...
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md px-6 md:px-10 py-6 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-border/50">
              ...
              onKeyPress={(e) => {
                if (e.key === "Enter") {
                  window.dispatchEvent(
                    new CustomEvent("open-ai", {
                      detail: { prompt: (e.currentTarget as HTMLInputElement).value },
                    })
                  );
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-4">
          ...
          <button
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent("open-ai", { detail: { prompt: "Help me log a new expense: " } })
              )
            }
        ...
        <section className="flex overflow-x-auto hide-scrollbar snap-x snap-mandatory -mx-6 px-6 pb-4 gap-4 md:grid md:grid-cols-6 md:mx-0 md:px-0 md:pb-0 md:gap-6">
          {[
            ...
            { label: "Total Spent", value: metrics?.totalExpenses, sub: `30 Day Period` },
            {
              ...
              sub: `${debts.length} Active Loans`,
              color: "text-red-500",
            },
            {
              label: "Savings Goals",
              value: metrics?.monthlySavings,
              sub: `${goals.length} Active Targets`,
            },
            {
              label: "Upcoming Bills",
              value: bills.filter((b: any) => b.status === "unpaid").length,
              isCount: true,
              sub: "Due this week",
            },
          ].map((metric, i) => (
            <div
              key={i}
              className="bg-surface border border-border p-6 rounded-[2.5rem] shadow-sm hover:shadow-md hover:border-primary/20 transition-all group relative shrink-0 w-[80%] md:w-auto snap-center"
            >
              <button
                onClick={() =>
                  window.dispatchEvent(
                    new CustomEvent("open-ai", {
                      detail: { prompt: `Help me manage my ${metric.label}: ` },
                    })
                  )
                }
              ...
              <p className="text-[10px] uppercase font-black text-text-secondary tracking-[0.2em] mb-4 group-hover:text-primary transition-colors">
                {metric.label}
              </p>
              <p
                className={`text-2xl font-black tracking-tighter mb-1 ${metric.color || "text-text-primary"}`}
              >
                {metric.isCount ? metric.value : `₹${metric.value?.toLocaleString("en-IN")}`}
              </p>
              <p className="text-[10px] font-bold text-text-secondary opacity-70 truncate">
                {metric.sub}
              </p>
            </div>
          ))}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-8">
              ...
              <div className="flex items-center justify-between mb-8 px-2">
                <div>
                  ...
                  <p className="text-xs font-bold text-text-secondary mt-1">
                    Velocity breakdown over past 7 days
                  </p>
                </div>
            ...
            <div className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm">
                  ...
                  <button
                    onClick={() =>
                      window.dispatchEvent(
                        new CustomEvent("open-ai", {
                          detail: {
                            prompt: "Analyze my recent spending activity and suggest edits",
                          },
                        })
                      )
                    }
              ...
              <div className="divide-y divide-border/50">
                {recentTx.length === 0 && (
                  <p className="text-sm text-text-secondary text-center py-10 font-bold">
                ...
                {recentTx.map((tx: Transaction) => (
                  <div key={tx.id} className="flex items-center gap-6 py-5 group">
                    ...
                    <p
                      className={`font-black text-base ${tx.type === "income" ? "text-emerald-500" : "text-text-primary"}`}
                    >
          ...
          <div className="lg:col-span-4 flex lg:flex-col overflow-x-auto lg:overflow-visible hide-scrollbar snap-x snap-mandatory -mx-6 px-6 pb-6 gap-6 lg:mx-0 lg:px-0 lg:pb-0">
            ...
            <div className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm shrink-0 w-[85%] lg:w-full snap-center">
              <h4 className="font-black text-lg text-text-primary mb-8 tracking-tight">
                Payment Hub
              </h4>
              <div className="grid grid-cols-2 gap-4">
                {accounts.map((wallet: any, i: number) => (
                  <div
                    key={i}
                    className="p-4 bg-background rounded-2xl group hover:border-primary/20 border border-transparent transition-all"
                  >
                    <p className="text-[8px] uppercase font-black text-text-secondary tracking-widest mb-1">
                      {wallet.name}
                    </p>
              ...
              <div className="space-y-4">
                {bills
                  ...
                  .map((bill: any, i: number) => (
                    <div
                      key={i}
                      className="p-4 bg-background rounded-2xl flex items-center gap-4 group hover:bg-surface border border-transparent hover:border-border transition-all"
                    >
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-primary/10 text-primary`}
                      >
                        <Zap className="w-5 h-5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-text-primary truncate">{bill.title}</p>
                        <p className="text-[10px] font-medium text-text-secondary mt-0.5 whitespace-nowrap">
            ...
            <div className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm shrink-0 w-[85%] lg:w-full snap-center">
              ...
              <div className="space-y-4">
                {goals.length === 0 && (
                  <p className="text-xs text-text-secondary font-bold text-center py-4">
                    No goals set.
                  </p>
                )}
                {goals.slice(0, 2).map((goal: any, i: number) => (
                  <div
                    key={i}
                    className="p-6 bg-background rounded-[2rem] border border-border relative overflow-hidden group hover:border-primary/30 transition-all"
                  >
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <h5 className="text-sm font-black text-text-primary tracking-tight">
                          {goal.title}
                        </h5>
            ...
            <div className="bg-surface border border-border rounded-[2.5rem] p-8 shadow-sm shrink-0 w-[85%] lg:w-full snap-center">
              ...
              <div className="space-y-4">
                ...
                {debts.slice(0, 3).map((debt: any, i: number) => (
                  <div
                    key={i}
                    className="p-4 bg-background rounded-2xl flex flex-col gap-3 group hover:border-red-400/30 border border-transparent transition-all"
                  >
                    <div className="flex justify-between items-center">
                      <p className="text-[10px] uppercase font-black text-text-secondary tracking-widest">
                        {debt.title}
                      </p>
                    ...
                    <button
                      onClick={() =>
                        window.dispatchEvent(
                          new CustomEvent("open-ai", {
                            detail: { prompt: `I want to pay ₹ for my ${debt.title} debt` },
                          })
                        )
                      }
              ...
              {metrics.categories.map((cat: any, i: number) => (
                <div key={i}>
                  ...
                  <div className="h-2 bg-background rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[#044E45] rounded-full"
          ...
          <div className="bg-[#044E45] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-xl flex flex-col justify-center">
            ...
            <div className="relative z-10">
              ...
              <p className="text-2xl font-bold leading-snug mb-8 max-w-md">
                Rahul, your spending on{" "}
                <span className="text-emerald-400 italic">Food & Drinks</span> is 14% lower than
