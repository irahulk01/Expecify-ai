...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [user, transactions, bills, goals, accounts, budgets, debts] = await Promise.all([
      ...
      billModel.findMany({ where: { userId } }),
      goalModel.findMany({ where: { userId } }),
      accountModel.findMany({ where: { userId } }),
      budgetModel.findMany({ where: { userId } }),
      debtModel.findMany({ where: { userId } }),
    ]);
    ...
    const totalBalance = (accounts as any[]).reduce((sum: number, acc: any) => sum + acc.balance, 0);
    const monthlySavings = (goals as any[]).reduce((sum: number, g: any) => sum + g.currentAmount, 0);
    ...
    const categoryColors = ["bg-orange-400", "bg-cyan-400", "bg-yellow-400", "bg-red-400", "bg-indigo-400", "bg-emerald-400"];
    const categories = Object.entries(categoryTotals).map(([label, amount], i) => ({
      label,
      amount,
    ...
    allTimeTx.forEach((tx: any) => {
      ...
      const idx = sixMonths.findIndex(m => m.month === txDate.getMonth() && m.year === txDate.getFullYear());
