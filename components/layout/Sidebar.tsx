...
export default function Sidebar({ user }: { user: any }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 bottom-0 w-64 bg-background border-r border-border py-8 flex flex-col z-50 transition-transform duration-300 md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {/* Mobile Close Button */}
        <button
          onClick={() => setIsOpen(false)}
        ...
        <nav className="flex-1 px-4 space-y-2">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 transition-colors ${isActive ? "text-primary" : "text-text-secondary group-hover:text-text-primary"}`}
                />
                {item.label}
              </Link>
            );
